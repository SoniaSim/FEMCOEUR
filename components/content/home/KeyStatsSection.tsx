...
export function KeyStatsSection({ stats, eyebrow, title }: KeyStatsSectionProps) {
  ...
  // 4 stats n'y passent qu'à partir de lg (2 colonnes avant), 2-3 stats dès sm.
  const dividerBreakpoint = count === 4 ? "lg:border-l" : "sm:border-l";

  return (
    <section aria-labelledby="key-stats-title" className="bg-secondary py-16 md:py-24">
      <div className="container">
        ...
        <ul className={`mt-14 grid gap-x-4 gap-y-12 ${gridClass}`}>
          {stats.map((stat, index) => (
            ...
            >
              <span className="whitespace-nowrap text-4xl font-black tracking-tight text-primary sm:text-6xl">
                {frTypo(stat.value)}
              </span>
